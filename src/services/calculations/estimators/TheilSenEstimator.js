...
export class TheilSenEstimator {
  ...
  static compute({ readings, tNow, thresholds }) {
    ...
    const thresholdResults = thresholds.map(threshold => {
      ...
      const bootstrap = BootstrapEstimator.run({ t, x, y, weights, threshold, tNow, maxT, iterations: BOOTSTRAP_ITERATIONS });
    ...
    return {
      ...
      // Signed per-axis rates (mm/week) — used for the always-visible
      // horizontal-vs-vertical rate comparison.
