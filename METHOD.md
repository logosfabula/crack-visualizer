...
**Movement-state icons.** Every floor's direction falls into one of 9 states — 3 horizontal outcomes (closing / still / expanding) × 3 vertical outcomes (rising / still / sinking). The Top 5 Soonest ETAs list shows a small icon per entry for this state: two wall blocks in elevation, where the gap between them encodes the horizontal state and their vertical offset encodes the vertical state — the same visual grammar a real crack-meter reading uses. When the Displacement Component selector is set to Horizontal-only or Vertical-only, the icon forces the other axis's state to "still" rather than showing a state that entry's ETA doesn't actually use (a horizontal-only projection never looks at the vertical trend). The icon set itself is swappable: it's resolved through a single `resolve(hState, vState)` contract (`src/components/common/icons/`), so a different visual style can be dropped in without touching anything that consumes it.
