...
### Additional Output
- **Angle Analysis**: Quadrant angle deviations from 90° orthogonality. Orthogonality of the crack meter's cross arms are relevant only as a litmus test for the actual reading. Since readings are prone to human error and usually have a .25mm approximation, they might result in non-orthogonal reconstruction of the virtual cross. Angle Analysis does not serve any other purpose than a control test of the reading itself and does not provide any information on the crack. In the dashboard, a deviation above 0.1° shows as a collapsed, clickable "Cross Angle Analysis" note (same collapse-in-place pattern as other disclosures, but orange to keep the severity visible) carrying a camera icon — a reminder that the alert scopes to the reading/capture process (freehand photo angle, or transcription), not to the wall's actual condition. It's deliberately not a diagnosis of which of the two: from a single reading's geometry, a camera-angle distortion and a transcription slip are indistinguishable.
...
- **Duplicate readings**: consecutive readings with an identical value (common when real movement is slower than the ~0.25mm reading precision) are collapsed to one point before fitting, so a long static stretch doesn't get counted as repeated evidence of zero movement. That collapsed point isn't treated as equivalent to a single, unconfirmed reading, though: it carries a confidence boost scaled to the square root of how many readings were absorbed into it (diminishing returns, matching how the standard error of a mean shrinks with the square root of independent observations) — 60 daily confirmations that a stretch held flat are stronger evidence of a genuinely flat trend than 2 readings 60 days apart, even though both must count as exactly one point toward the pairwise-slope median. Because pairwise weight is the *minimum* of the two points' weights, this boost only compounds between two well-confirmed points — one heavily-reconfirmed point can never outweigh a pairing with a less-confirmed one, so it can't hijack the trend on its own. Collapsing a run always keeps its later member's value and confirmation count, but the series' own first reading (t=0, the regression's time origin) is preserved even if it's itself part of a leading run — otherwise the fit's observed window would silently shrink by however long that opening flat stretch lasted, inflating the fitted rate.
- **ETA from the fitted trend**: the crossing time for a threshold is found by solving where the fitted 2D line intersects the threshold's displacement circle. If the fitted trend has no directional slope at all, or its closest approach to the origin never reaches the threshold, or the crossing would fall more than 100 years from today, the result is reported as "not reached on current trend" rather than a forced number. That horizon is fixed, not scaled to how long the floor has been monitored — a longer observation history doesn't earn a longer trusted projection, since the cap is about how far a straight-line projection is plausible at all.
...
- **Bootstrap range and consensus**: a bootstrap resampling of the fit (500 replicates) gives two distinct outputs, not one — a genuine 90% confidence range for the ETA date (the middle 90% of resampled crossing times), and separately, the *consensus*: what share of those same resamples agree the threshold is reached at all. The consensus isn't a confidence bound on any number — it's a tally of how many resamples agree with the reached/not-reached verdict — useful when the point estimate is near the boundary of "will this happen," since a low consensus flags a fragile verdict even when the date range itself looks narrow.
...
### Horizontal vs. Vertical Rate

Both methods fit `x(t)` and `y(t)` independently, so a floor's horizontal and vertical rates are always available alongside its combined 2D ETA — useful for judging whether, say, sinking (vertical) or outward wall pull (horizontal) is the stronger driver for a given crack.

- Shown as a single vector-arrow figure per floor: horizontal and vertical rate (mm/week) plotted as one arrow from the origin, angled to the actual H/V ratio and colored by whichever axis currently has the larger magnitude — angle and length read the comparison in one glance, without comparing two separate marks. A floor with no net movement on either axis draws as a small open circle instead of an arrow. Numeric values and direction words ("expanding"/"closing", "rising"/"sinking") are given below the figure. A cross-floor grid of the same per-floor arrow appears in the Structural Analysis Summary. Every arrow on the page shares one fixed axis scale — the largest |rate| currently observed across every floor and axis for the selected ETA method, rounded up to a clean reference value — so a given rate always renders at the same arrow length wherever it's shown, and the axis labels themselves state the scale rather than leaving it implicit.
- The ETA panel itself (Trend Rate, threshold crossings, Top 5 Soonest ETAs) always projects the combined 2D displacement — the per-axis rate figure already surfaces the decomposition, so there's no separate single-axis ETA to switch to.

**Movement-state icons.** Every floor's direction falls into one of 9 states — 3 horizontal outcomes (closing / still / expanding) × 3 vertical outcomes (rising / still / sinking). The Top 5 Soonest ETAs list shows a small icon per entry for this state: two wall blocks in elevation, where the gap between them encodes the horizontal state and their vertical offset encodes the vertical state — the same visual grammar a real crack-meter reading uses. The icon set itself is swappable: it's resolved through a single `resolve(hState, vState)` contract (`src/components/common/icons/`), so a different visual style can be dropped in without touching anything that consumes it.
