...
- **Horizontal vs. Vertical Rate** is shown at all times, independent of the selector, as a diverging bar chart per floor (the two signed per-axis rates, mm/week, growing left/right from zero) plus a note on which one currently has the larger magnitude — so the relative-strength comparison doesn't require switching the selector back and forth. A second grouped bar chart in the Structural Analysis Summary compares all floors' horizontal and vertical rates side by side, for the cross-floor view the per-floor chart can't give.
