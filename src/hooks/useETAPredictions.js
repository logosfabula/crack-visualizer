...
import { ETA_THRESHOLDS_MM, ETA_COMPONENTS } from '../constants/regressionConfig';
...
export const useETAPredictions = (processedData) => {
  return useMemo(() => {
    ...
    Object.keys(METER_CONFIGS).forEach(meterKey => {
      ...
      const currentHorizontalDisplacement = Math.abs(last.x);
      const currentVerticalDisplacement = Math.abs(last.y);
      const tNow = (new Date() - firstDate) / DAY_MS;

      const currentValueFor = (component) => {
        if (component === ETA_COMPONENTS.HORIZONTAL) return currentHorizontalDisplacement;
        if (component === ETA_COMPONENTS.VERTICAL) return currentVerticalDisplacement;
      ...
      Object.values(ETA_ESTIMATORS).forEach(estimator => {
        const components = {};
        let componentRates = null;

        Object.values(ETA_COMPONENTS).forEach(component => {
          const raw = estimator.compute({ readings, tNow, thresholds: ETA_THRESHOLDS_MM, component });
          if (!raw) {
            components[component] = null;
            return;
          }
          if (!componentRates) componentRates = raw.componentRates;

          const currentValue = currentValueFor(component);
          const thresholds = raw.thresholdResults.map(tr => {
            if (currentValue >= tr.threshold) {
          ...
          components[component] = { rateMmPerWeek: raw.rateMmPerWeek, thresholds, direction: raw.direction };
        });

        estimates[estimator.id] = { components, componentRates };
      });
      ...
      results[meterKey] = {
        ...
        rawReadingCount,
        currentDisplacement,
        currentHorizontalDisplacement,
        currentVerticalDisplacement,
        estimates
      };
    });
